# [「转」openYuanrong 数据系统：近计算高性能分布式内存缓存](https://mp.weixin.qq.com/s/p6GaMEU5Y0nDj_TTAHW6lA)

*车煜林*[OpenAtom openEuler](javascript:void%280%29;)*2026-02-11 17:55:51广东*
...
![](https://mmbiz.qpic.cn/mmbiz_png/lzCickRegvlocMfDibHIsHhoWIzAwKboSicNNLtynREehZoRHIKyg7PcB3ic0BCOrUHQBic4gZbDw6BZwzKuBekRtmA/640?wx_fmt=png&from=appmsg)
...
- - 作为顶层参数传递：在函数间直接传递引用，无需等待数据就绪。
  - 作为嵌套参数传递：将引用嵌入复杂数据结构中，进一步减少不必要的数据复制。
    ...
    ![](https://mmbiz.qpic.cn/mmbiz_png/lzCickRegvlocMfDibHIsHhoWIzAwKboSicuNQxFzHptia07wGFiaAfhdADc8NV7DSxMIDicP8P2ChicICdO4ZVv7SrFQ/640?wx_fmt=png&from=appmsg)

<!--THE END-->

- - 键值（Key-Value）接口： 开发者可通过指定Key来读写、删除具有唯一标识的命名对象，适用于需要显式命名的场景。
    
    ![](https://mmbiz.qpic.cn/mmbiz_png/lzCickRegvlocMfDibHIsHhoWIzAwKboSicbR7HuTy0biarvO2I9cibfB9zo9ls4pwRBqXfuWfSmTF2w7N8S8rtzXUA/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/lzCickRegvlocMfDibHIsHhoWIzAwKboSicR2MMh2icSlSZ2qNuAsQicl6UicrqBjcnDbLhBgGvy9567b3iadRdusOjWQ/640?wx_fmt=png&from=appmsg)
...
- 挑战二：数据位置与生命周期管理负担重
  ...
  ![](https://mmbiz.qpic.cn/mmbiz_png/lzCickRegvlocMfDibHIsHhoWIzAwKboSicUjUWGpU7Gt6W7jN4MXSULdYWOkVULA1ib0fhYO48k3cV99f23WBGZzQ/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/lzCickRegvlocMfDibHIsHhoWIzAwKboSicvh7DgZs2m4cks8MWbib1hR4kg90IrB2Q53uU6R6npvUaZEqZjOzKoiaw/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/lzCickRegvlocMfDibHIsHhoWIzAwKboSicbxia04u8O9bHS3LhsAcvDeFVia0nUhEDUqSSF1DWtJibxB5yMeImtQ5GQ/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/lzCickRegvlocMfDibHIsHhoWIzAwKboSicOBiahAgRA44wR0mJaI61wVp4w0TiciaoQdNnZibD1e4V2lTmuf006VudhQ/640?wx_fmt=png&from=appmsg)

### LLM实例快速弹性

数据系统支持大模型推理实例快速弹性，利用模型参数的只读性，从已有服务实例的NPU快速复制模型参数到新实例。

![](https://mmbiz.qpic.cn/mmbiz_png/lzCickRegvlocMfDibHIsHhoWIzAwKboSiccj8zHjE04to62KcLs3DDmr2BPrr6SL4NVibW0YWPKjXHk4asHIhrscA/640?wx_fmt=png&from=appmsg)

经过实验验证：

- 单推理实例推理服务实例弹性能力提升20~100x至5秒内。
  
  ![](https://mmbiz.qpic.cn/mmbiz_png/lzCickRegvlocMfDibHIsHhoWIzAwKboSicfZsldQ185cw6zWZ36a0BTwh70BW1ibPf7DDbAN0QYbBjO1qh89rZs6w/640?wx_fmt=png&from=appmsg)
- 借助数据系统P2P数据负载均衡优化，多实例批量弹性速度接近单实例，D2D数据流转不存在单点瓶颈。
  
  ![](https://mmbiz.qpic.cn/mmbiz_png/lzCickRegvlocMfDibHIsHhoWIzAwKboSicQ3a5Aic8C63ibmibDUGsaRY1oicR1wdZiawvaxlF5TXekyXczibSYEDdw5gA/640?wx_fmt=png&from=appmsg)
...
\[1] 官网地址：http://docs.openyuanrong.org/

\[2] 源码地址：https://atomgit.com/openeuler/yuanrong

\[3] 问题反馈：https://atomgit.com/openeuler/yuanrong/issues

# [iSulad 与 Kuasar 双剑合璧：让你单节点轻松运行 10w+ 个 WASM 沙箱](https://mp.weixin.qq.com/s/Ysu13B_JjQ84g6TmS1fvBg)

[OpenAtom openEuler](javascript:void%280%29;)*2023-11-22 17:30:00*
...
![](https://mmbiz.qpic.cn/mmbiz_png/A0h5yD51CMaeo7VMyuEQTpDugyHI5XgTX19tXGvZNKcs0wSt1kDMaRgFyXKvxwdfoabic4X03ncRDKdWdAia3gnA/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/A0h5yD51CMaeo7VMyuEQTpDugyHI5XgT8LNbS8nzRarHhLKglQ24Kyv0AkgURTwovsoAw6EBeWXcKM0972ATsg/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/A0h5yD51CMaeo7VMyuEQTpDugyHI5XgTGcSflk6hlicvibXQLib9Lv0ibTpIuW7cF5QHNXUozRMDNjPTS1dTAKU1mg/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/A0h5yD51CMaeo7VMyuEQTpDugyHI5XgTAuhfIdsicN6Y3jPJXaAD0USS7GhLmMmRChAno5ND1cuydn9QakkK5IQ/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/A0h5yD51CMaeo7VMyuEQTpDugyHI5XgTrE6KfbVEj9m1argKNMaF1qAQS5T2frSZmhNibKudsb0226F8kEmjchw/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/A0h5yD51CMaeo7VMyuEQTpDugyHI5XgTnGrZr2Uwapuh69jRAhnibgc4LcMuxnKvVwRyBuQjCzQiahoPbYvLd4uQ/640?wx_fmt=png&from=appmsg)
...
Kuasar 对接WasmEdge方案介绍

![](https://mmbiz.qpic.cn/mmbiz_png/A0h5yD51CMaeo7VMyuEQTpDugyHI5XgT4F3ujYYxVjiaevnED31dN1Os3gNAZ0TsvEV6gHWuH73sBnteM4wTdCQ/640?wx_fmt=png&from=appmsg)
...
```
# 以 runwasi 项目中 wasi-demo-app 应用为例介绍$ git clone https://github.com/containerd/runwasi.git$ cd runwasi/crates/wasi-demo-app# 安装 rust wasm32-wasi 编译工具链$ rustup target add wasm32-wasi# 将 "wasi-demo-app" 程序编译成 wasm 目标格式$ cargo build --target=wasm32-wasi --release# 编译生成的 wasm 模块会存放在 target/wasm32-wasi/release/ 目录下$ ls runwasi/target/wasm32-wasi/release/*.wasm                                  target/wasm32-wasi/release/wasi-demo-app.wasm
```
...
```
# 下载 kuasar 项目源码$ git clone https://github.com/kuasar-io/kuasar.git# 切换到 wasm沙箱模块源码目录下$ cd kuasar/wasm# 编译 wasm-sandboxer 组件$ cargo build --release --features=wasmtime# 生成的 wasm-sandboxer 二进制可执行文件路径$ ls target/release/wasm-sandboxer
```
...
```
// 在 iSulad 容器引擎的配置文件 /etc/isulad/daemon.json 中添加下面有关 wasm 沙箱运行时的配置信息{  ......  "cri-runtimes": {    "wasm": "io.containerd.wasm.v1"  },  "cri-sandboxers": {    "wasm": {        "name": "wasm",        "address": "/run/wasm-sandboxer.sock"      }  },  ......}
```
...
```
$ cat run_example_wasm_container_with_isulad.shset -e# Prepare for the wasm Pod and Container config file.touch pod.json container.jsoncurrent_timestamp=$(date +%s)cat > pod.json <<EOF{    "metadata": {        "name": "test-sandbox$current_timestamp",        "namespace": "default"    },    "log_directory": "/tmp",    "linux": {        "security_context": {            "namespace_options": {                "network": 2,                "pid": 1            }        }    }}EOFcat > container.json <<EOF{    "metadata": {        "name": "wasm_$current_timestamp",        "namespace": "default"    },    "image": {      "image": "wasi-demo-app:latest"    },    "command": [       "/wasi-demo-app.wasm","daemon"    ],    "log_path":"wasm.log",    "linux": {        "security_context": {            "namespace_options": {                "network": 2,                "pid": 1            }        }    }}EOF# Run a wasm containercrictl run --runtime="wasm" --no-pull container.json pod.jsonrm -f container.json pod.json
```

执行上述脚本程序后，可以通过crictl命令观测到 WASM 沙箱和 WASM 函数的运行的状态。

```
$ crictl ps -aCONTAINER           IMAGE                  CREATED             STATE               NAME                ATTEMPT             POD ID              POD0b1dee6dc1a6c       wasi-demo-app:latest   2 seconds ago       Running             wasm                0                   37fe4fc2fc358       unknown$  crictl podsPOD ID              CREATED             STATE               NAME                     NAMESPACE           ATTEMPT             RUNTIME37fe4fc2fc358       27 seconds ago      Ready               test-sandbox1700546390   default             0                   (default)
```
...
![](https://mmbiz.qpic.cn/mmbiz_png/A0h5yD51CMaeo7VMyuEQTpDugyHI5XgTKQuJvSPj3y55oQaSPBnZPiaeI5icG8qicYrXwUsGN8ll06wQicIyI8Ph6A/640?wx_fmt=png&from=appmsg)

图 8. wasm-sandboxer 运行时进程内存开销统计

![](https://mmbiz.qpic.cn/mmbiz_png/A0h5yD51CMaeo7VMyuEQTpDugyHI5XgTRAVHiaQluPwFuFVW6wUIkQMXV4qyibGcH2fCtNeEpO0ibIRWSIRUvyUPA/640?wx_fmt=png&from=appmsg)

# [SIG-Long 异构融合 - 基于灵衢的内存池化借用](https://mp.weixin.qq.com/s/8T6CK3ODpC9bCDW_QiKsrw)

[OpenAtom openEuler](javascript:void%280%29;)*2025-12-18 18:01:00广东*

## 背景

OpenAtom openEuler （简称“openEuler”或 “开源欧拉”） 作为全场景的开源操作系统，是连接底层硬件和上层应用的桥梁。在计算领域，面对异构硬件（CPU+XPU）、高速互联总线（UB、CXL 等）以及通智算融合负载（大数据，生成式推荐，AI 推理等）的演进和挑战，其核心任务是在资源与负载之间，扮演关键的总调度和总指挥角色，实现硬件资源和软件负载的精准匹配和按需供给，融合异构算力，构建异构融合计算基础设施框架。
...
1. 集中池化架构 -- 中心池化
   ...
   ![](https://mmbiz.qpic.cn/mmbiz_png/jqTxnpIZ2mBCgibhMGibUgOCR4T8YhR1ZkWnE1PibNn24Ae9lKfcdXWxIWicTict46giaI36rVS52m7x4icGV0Ab99TTQ/640?wx_fmt=png&from=appmsg)
   
   - **技术特征**：共享内存来源于同构的内存服务器。
   - **成本&TCO**：需要新增单独内存池硬件，影响成本和 TCO；当计算节点内存不够时，扩展更灵活。
   - **可靠性**：计算节点与内存节点分离，计算节点故障不影响内存数据；内存节点通过内存 mirror、亚健康检查、故障时内存迁移等手段保证高可靠。
2. 对等池化架构 -- 逻辑池化
   
   ![](https://mmbiz.qpic.cn/mmbiz_png/jqTxnpIZ2mBCgibhMGibUgOCR4T8YhR1ZkYiaNEMKEibSHsyPtAiaTibpUPa606pwI4OVibzcbLoiaNpQOhOSCm89oNAicQ/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/jqTxnpIZ2mBCgibhMGibUgOCR4T8YhR1ZkgViaGichn0vSGTTNxU9xHqsl4N5hAuI8p343mJmlPwicQDb0PJFXD0ILg/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/jqTxnpIZ2mBCgibhMGibUgOCR4T8YhR1ZkbhFKUpviccZPLP9h534epHgIwOibgOXakVxVkhicRzf1iaWOCzXwInEDeA/640?wx_fmt=png&from=appmsg)

#### 内存共享

![](https://mmbiz.qpic.cn/mmbiz_png/jqTxnpIZ2mBCgibhMGibUgOCR4T8YhR1Zk0GTngYQHCh0cvZu7q8QLrMSZxOibpt2Heib8cmWCVE2K3Vq0b279oMhw/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/jqTxnpIZ2mBCgibhMGibUgOCR4T8YhR1ZkpQxz1th1LnglWRTdxPOm4nu11NUjw9da0qm4TBpfBA4ibDjr1CkibibLQ/640?wx_fmt=png&from=appmsg)
...
我们将上述“数据一致性维护”抽象为 **set\_ownership** 接口。OBMM 对于 set\_ownership 提供了参考实现，细节参考下文 OBMM 的 set\_ownership 小节。

将 set\_ownership 定义为：

```
__weak int set_ownership(...) { return 0; }
```

当 ScaleUP 互联具有 Cache Coherence 能力时，可直接在相应的运行环境中不额外实现 set\_ownership 接口。
...
我们不妨将这个操作接口命名为：\_mp\_export；

> OBMM 提供的基于灵衢的参考实现 obmm\_export 见下文。
...
我们不妨将这个操作接口命名为：\_mp\_import；

> OBMM 提供的基于灵衢的参考实现 obmm\_import 接口见下文。

此处所述 [#f]() 一般由上文所述的超节点链路管理组件 FM 实现；此处 import 操作调用 FM 的功能即可。
...
OBMM 基于灵衢实现了 set\_ownership 接口，用来在内存借用和共享时维护数据的一致性操作。具体实现包括：

● 缓存回刷到 persistent point

● 阻止预取

具体内容可参考：https://gitee.com/openeuler/obmm/blob/master/doc/obmm\_set\_ownership.md

相关实现为内核态实现，可参考内核代码仓库：https://gitee.com/openeuler/kernel

### 内存借用 - 接口 & 用例

OBMM 实现的 obmm\_export 和 obmm\_improt 提供了 libobmm 库封装，代码链接：https://gitee.com/openeuler/obmm/tree/master/src/libobmm
...
int export_interface-_demo(void)
{
...
int export_useraddr_interface-_demo(void)
{
	unsigned int device_deid = 0x101;
	int ret;
	mem_id id;
...
> 引用自：https://gitee.com/openeuler/obmm/blob/master/doc/obmm\_export.md
...
> 引用自：https://gitee.com/openeuler/obmm/blob/master/doc/obmm\_import.md
...
● 通过内存借用构建大规模内存池，消减数据库大数据等业务在内存和 I/O 之间的交换，提升业务吞吐量约 15%

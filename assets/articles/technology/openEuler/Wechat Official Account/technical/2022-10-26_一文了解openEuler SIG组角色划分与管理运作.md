# [一文了解openEuler SIG组角色划分与管理运作](https://mp.weixin.qq.com/s/9RWLnrUrkSj6NugcGNqltw)

[OpenAtom openEuler](javascript:void%280%29;)*2022-10-26 18:30:00*
...
![](https://mmbiz.qpic.cn/mmbiz_png/A0h5yD51CMaWdoavsYAP7qqfMYvl12opQ8pibafph5PlzJ96tkiaUFUvialvPLgPpg5XMlhUouCh7l5ibwwIFan7aA/640?wx_fmt=png)
...
**SIG组的牵引者、规划者，需做好SIG组的发展和演进；**

负责定期组织SIG组会议、并代表SIG组向社区展示SIG组的运作情况；

Maintainer 的权限默认覆盖其所在的SIG组的所有代码仓，包含所有新增加到该SIG的代码仓。

Committer

**SIG组部分仓库的看护者，是这部分仓库的第一责任人；**

Committer管理指定的仓库，其权限覆盖明确的部分代码仓，权限默认不会扩展。

Contributor

**SIG组部分仓库的重要贡献者，也是这部分仓库Committer的后备力量；**

通常也是这部分代码仓库问题主要修复者和代码开发者。

Developer

**社区最广泛的代码贡献者；**

在社区长期贡献可通过自荐或被推荐为仓库Contributor，甚至Committer。

RepoAdmin

**通常兼任SIG组的Maintainer，对这部分仓库的代码合入全量负责；**

社区部分自研代码仓库可设置Admin权限,RepoAdmin对这部分仓库具有管理员权限，可通过git push合入代码。    
...
(sig\_info.yaml文件是社区成员管理的载体)

SIG组成员管理

上文中提到的sig\_info.yaml文件是社区成员管理的载体。因此，每个SIG组都需要新增一份sig\_info.yaml文件以便进行SIG管理。

sig\_info.yaml文件主要由Maintainer和Committer编写，具体的编写方式可以查看：

https://www.openeuler.org/zh/blog/georgecao/openEuler-sig-member-management.html

在进行sig\_info.yaml 文件配置时，需要包含：
...
注意：sig\_info.yaml 文件已与OWNER文件进行兼容。只有当前SIG目录下OWNER文件被删除后，sig\_info.yaml信息才会生效；OWNER文件存在时则继续保持原成员关系和权限管控功能。

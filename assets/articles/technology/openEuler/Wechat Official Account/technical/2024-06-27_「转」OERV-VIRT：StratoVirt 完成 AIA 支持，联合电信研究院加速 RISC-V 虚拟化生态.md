# [「转」OERV-VIRT：StratoVirt 完成 AIA 支持，联合电信研究院加速 RISC-V 虚拟化生态](https://mp.weixin.qq.com/s/pNlT6FrV3FGjs6YTMYB8bw)

*OERV-VIRT*[OpenAtom openEuler](javascript:void%280%29;)*2024-06-27 17:58:01中国香港*
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/tkVlnC85sFHLXXfCic5KymjEkTmuR3brDKKjH5iavGdL7ZMjpFBY6FThV8MycreaeXQ74qhqm11exRbXKHaUNYdQ/640?wx_fmt=png&from=appmsg)
...
1. 为 util crate 补全了设备树生成过程中所需的 PHANDLE 常量定义，并针对 RISC-V 架构调整了 seccomp 的相应逻辑，为虚拟机的内核提供了安全屏障
2. 为machine\_manager crate 新增了支持 RISC-V MicroVM 主板和 StandardVM Virt 主板的功能，使得用户可以根据需要选择不同的虚拟机板型
3. migration crate 的改进包括为 RISC-V 特有的中断设备 AIA 添加了快照逻辑，使得这些设备在进行虚拟机迁移时能够保持状态一致性
4. 为 cpu crate 加入了管理 RISC-V vCPU 特性和状态所需的结构体和方法，提高了用户客制化 vCPU 的粒度
5. 为 acpi crate 加入了适用于 RISC-V 的 madt-subtable 定义，为 StandardVM 的工作提供了支撑
6. 在 devices crate 中加入和调整了适合 RISC-V 特有的 AIA 设备管理和操作接口，同时对 fwcfg 和 serial 等设备进行了必要的修改
7. hypervisor crate 参考了 riscv64 平台上的 kvm.h 头文件定义，通过支持了 RISC-V 的 rust-vmm crates 实现了与 KVM 模块的交互接口
8. 为 bootloader crate 加入了根据 RISC-V image header 实现的内核快速加载逻辑，提高了启动效率
9. 为 machine crate 补全了 RISC-V MicroVM 的内存分布、设备树生成逻辑及其启动流程，这为虚拟机的运行提供了基本架构
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/tkVlnC85sFHLXXfCic5KymjEkTmuR3brDw9bUr90iakM4wj4sv2f83xuGKxXibj1n9NSx8ZnocOe93DuicuLsyE8Sw/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/tkVlnC85sFHLXXfCic5KymjEkTmuR3brDic0iavInic6RfdF9rXxDP4KaUKjCN6H2smoReJQsHEGTT5ETvwyVuoFiaw/640?wx_fmt=png&from=appmsg)
...
2. 继续使能 kym-bindings、kvm-ioctls 、acpi-tables 和 linux-loader对 RISC-V 架构的支持工作，完善 Rust 虚拟化的CI链路
3. 对标完成 StratoVirt 的 StandardVM 开发工作
4. 进一步对 Dragonball、Cloud-Hypervisor 和 Firecracker 等 Rust Hypervisors 引入 RISC-V 架构的支持，并且引入 openEuler 社区

![](https://mmbiz.qpic.cn/sz_mmbiz_png/tkVlnC85sFHLXXfCic5KymjEkTmuR3brDkGSKAPOJlYfhxsy9AC79Yic1wswEafoQahfFgj9eoOzNxDvX9a1xYMA/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_jpg/tkVlnC85sFFUHZTf4iak6JuwtHYf43cK4RL4VuPoooQN9yI8BbCGB640I4DACq54zcmggIY8d2TdzfW9CWJXcgA/640?wx_fmt=jpeg&from=appmsg)

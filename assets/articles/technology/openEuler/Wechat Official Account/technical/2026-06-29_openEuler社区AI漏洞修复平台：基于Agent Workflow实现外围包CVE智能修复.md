...
原创*陈佳一*[OpenAtom openEuler](javascript:void%280%29;)*2026-06-29 17:54:11广东*
...
OpenAtom openEuler（简称 “openEuler” 或 “开源欧拉”） AI漏洞修复平台是openEuler社区使用AI修复内核CVE和外围包CVE平台。上一篇[《](https://mp.weixin.qq.com/s?__biz=MzkyMjYzNjU0Ng%3D%3D&mid=2247520192&idx=1&sn=b2966176741559adfc13eb32cadc97e0&scene=21#wechat_redirect)[openEuler社区AI漏洞修复平台：通过PatchFlow Agent完成内核CVE漏洞修复》](https://mp.weixin.qq.com/s?__biz=MzkyMjYzNjU0Ng%3D%3D&mid=2247520192&idx=1&sn=b2966176741559adfc13eb32cadc97e0&scene=21#wechat_redirect)着重介绍了内核CVE漏洞修复。本篇将着重介绍外围包漏洞修复。

●平台链接：https://gitcode.com/openeuler/nvwa-cve-fixer 

openEuler社区持续推进海量软件包的智能化运维效能提升，其中面向外围包CVE修复场景，由“Agent编排+Harness工程”驱动的Agent Workflow智能化补丁作业系统已在**600+外围包仓库**完成落地应用，支撑超过110+成功合入。为方便介绍，**下文将这套外围包CVE补丁作业系统简称为PatchHarness。**
...
**问题切入：海量补丁智能化回合面临哪些现实挑战**

外围包由于其独特的代码组织模式，相比内核仓的CVE修复流程增加了许多前后置流程，这也给PatchHarness的设计带来了新的挑战：

![](https://mmbiz.qpic.cn/sz_mmbiz_png/rxr9tddEHOvsAe5qKXIfVRVgiciaKcKw2K1PTY2GQffuvqA9I1jVXHKly62MJKVzDicIB1ht8vb5oPlesbtK15OWkicagxFQxlpBtyCJJJXyib8o/640?wx_fmt=png&from=appmsg)

\*本图由AI生成，仅供示意

### **1.确定性和不确定流程混合，易引发代码幻觉**
...
**能力构建：****构建稳定、可靠、可扩展的Agent Workflow补丁作业系统**
...
- **执行引擎（编排）：**以 patch-workflow.sh 作为唯一入口，支持通过 6 步流水线进行场景感知与流程编排。系统具备断点续传能力，可通过状态文件记录完成状态，支持从任意中断步骤恢复执行。
- **CVE流水线（混合引擎）：**采用 “AI + 确定性混合引擎”。在补丁发现、影响分析、代码适配、级联补丁搜索等关键环节调用大模型能力；在构建验证、比较、Spec 修改等确定性环节利用传统脚本，兼顾效率与可靠性。

**架构核心理念：**

入口分发层持续感知，执行引擎支持断点续传，CVE流水线融合 AI 与确定性脚本。

![image.png](https://mmbiz.qpic.cn/sz_mmbiz_png/rxr9tddEHOsoLCOCDOmAPBpibicCFDwSELo9rvXWb4haNib7V5pLcIiaYENUficjjCPTTs4GIJRiaxicU1iaiboJFuIibbImR6mzAIib2Ny2HlwFzaW0iaU/640?wx_fmt=png&from=appmsg)
...
PatchHarness提供CVE修复的触发命令，使用格式为/branches &lt;branch1&gt;,&lt;branch2&gt;,...，比如/branches master表示对外围包master分支进行CVE修复：

![image.png](https://mmbiz.qpic.cn/mmbiz_png/rxr9tddEHOsx9eDU2ZvVzZeHbX6TcIRp4HuJwtcIwXQTwhXFDELShDs4TibIr93ZMJFcalpVWCZHaRDq6k5mVtSRmMoEtFhqgptGvnvZNbcc/640?wx_fmt=png&from=appmsg)

![image.png](https://mmbiz.qpic.cn/mmbiz_png/rxr9tddEHOtdZsHIPxLCqW117cntxKISpJwZWEKtrzzzK0KjM995bwF62a6DEjwQXFTsTBp2ItdT7p9GDYz6Z67tFiaOU4ia0jWtdWpn8V1Fo/640?wx_fmt=png&from=appmsg)

### **2. CVE影响性分析结果反馈**

评论后触发自动化处理任务，进行CVE补丁搜索以及影响性分析。任务端到端完成后，PatchHarness会通过评论区反馈漏洞分析结果以及PR链接：

![image.png](https://mmbiz.qpic.cn/mmbiz_png/rxr9tddEHOsWAVEzZGs5eMotIMQ8u9FC3N35PibIXfrbnklH2RicWf6mKUTXKTYBIxaMpibtAaz1zCxQia47fvgL14TA0oiaqBZsjSF1QmmdTVM4/640?wx_fmt=png&from=appmsg)

![image.png](https://mmbiz.qpic.cn/mmbiz_png/rxr9tddEHOvcHicOxtDelxa9Uk5oPxExB7shFaGnzK1RItGbIwe2uS7QYZsQXnTovc4CaOwAQEYAeDbicrmwv6Dc8nQTJ1Z7MhCdH80mOicvzA/640?wx_fmt=png&from=appmsg)

### **3. 补丁适配报告反馈**

CVE修复补丁回合完成后，会在评论区反馈补丁适配报告：

![image.png](https://mmbiz.qpic.cn/mmbiz_png/rxr9tddEHOsVUUPF9OgGRwMF49uqn3xu6nX666kTicxBVb3rxeia5X9JSqekyUxw8GDXnlZTw14iaB3MjRS6aT470FJxhc5DI5bkIPbKmIg8BQ/640?wx_fmt=png&from=appmsg)

### **4. 可视化看板**

PatchHarness还提供了一套可视化看板，支持细粒度跟踪当前执行任务的流水线所处的环节，实时显示过程中输出的日志信息：

## ![image.png](https://mmbiz.qpic.cn/sz_mmbiz_png/rxr9tddEHOvNONmicicDDDibwceTOoVgW9Niah9iaGGNFhG1icw6BCBjq7wKKcnnicLJ1m0loNcUnqxqtPlftpiboGr18NUWpaicCzLdZmxyHKywdUTI/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_gif/rxr9tddEHOsbX3zeXw0TYrhvnBpL2GveibCvtIdpFFdK9NDhEQgcR8zBTroLN9stA2bNvp2NYu63uXk5ffSUibcnzACiaXt35AeRyeIt0FT99Q/640?wx_fmt=gif&from=appmsg)
...
![image.png](https://mmbiz.qpic.cn/mmbiz_png/rxr9tddEHOvX0OqiaFnHiaw1R796QvwKKm85rEziaZbU5CSoibRENoOiaZYJF2ljcm1o6HsJMuXq00gzAUCWU5AexhUY7wsIgelyGwccoWicpYYoM/640?wx_fmt=png&from=appmsg)

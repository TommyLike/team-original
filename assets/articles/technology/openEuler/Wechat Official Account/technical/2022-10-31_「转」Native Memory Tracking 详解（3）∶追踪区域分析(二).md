# [「转」Native Memory Tracking 详解（3）∶追踪区域分析(二)](https://mp.weixin.qq.com/s/iZ-yET_bIcBfmEsZJGBmjg)

*窦义望*[OpenAtom openEuler](javascript:void%280%29;)*2022-10-31 20:00:00*

上篇文章 [Native Memory Tracking 详解（2）:追踪区域分析(一)](http://mp.weixin.qq.com/s?__biz=MzkyNTMwMjI2Mw%3D%3D&mid=2247489471&idx=1&sn=11fb0d30c8bcea50b1bda0b1cd2a6595&chksm=c1c9fb27f6be72314b28d163146ecaa11b298ac9856625cfdc8909d464be6d8b220ef615fd69&scene=21#wechat_redirect) 中，分享了NMT追踪区域的部分内存类型——Java heap、Class、Thread、Code 以及 GC，本篇图文将介绍追踪区域的其它内存类型以及 NMT 无法追踪的内存。
...
PerfData 是 JVM 中用来记录一些指标数据的文件，如果开启 -XX:+UsePerfData（默认开启），JVM 会通过 mmap 的方式（即使用上文中提到的 os::reserve\_memory 和 os::commit\_memory）去映射到 `{tmpdir}/hsperfdata_<username>/pid` 文件中，jstat 通过读取 PerfData 中的数据来展示 JVM 进程中的各种指标信息.
...
> 需要注意的是，Unsafe\_AllocateMemory 分配的内存在 JDK11之前，在 NMT 中都属于 Internal，但是在 JDK11 之后被 NMT 归属到 Other 中。例如相同 ByteBuffer.allocateDirect 在 JDK11 中进行追踪：`[0x0000ffff8c0b4a60] Unsafe_AllocateMemory0+0x60[0x0000ffff6b822fbc] (malloc=393218KB type=Other #3)`
...
![](https://mmbiz.qpic.cn/mmbiz_jpg/icntuIQtpSJWvG5YZw5H1ky3nfPrT6f8er9l0teJRRWGkiavmJI0PGW9ezf3f34gTNQsUx70ciawOib8RRic67eLILQ/640?wx_fmt=jpeg)

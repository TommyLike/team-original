# [「转」编译器优化那些事儿（6）：别名分析概述](https://mp.weixin.qq.com/s/kSlagH8CmiscZ14Rg-4GYA)

*邢玉帅*[OpenAtom openEuler](javascript:void%280%29;)*2022-09-13 18:00:00*
...
![](https://mmbiz.qpic.cn/mmbiz_png/icntuIQtpSJVjRR5o4mRzKlicGTPEpbiaW6A1BdyfMIPAvSsCHEugibGsf5DhTEBia3xMIVdib2DwpL4YonbpZUoms5A/640?wx_fmt=png)

**域基础分析**：仅对结构体中的成员进行建模，而不感知对象。其建模后的结果如下图，仅有\*.field1和\*.field2：

![](https://mmbiz.qpic.cn/mmbiz_png/icntuIQtpSJVjRR5o4mRzKlicGTPEpbiaW6qljljLDoPibHVFuFrdRib6sGKR0o1Dl9VtQvWKHFK1EdfVcwxZmwbpPA/640?wx_fmt=png)

**域敏感**：既对对象建模，又对成员变量进行处理。其建模后的结果如下图，有a1.field1、a1.field2、a2.field1、a2.field2：

![](https://mmbiz.qpic.cn/mmbiz_png/icntuIQtpSJVjRR5o4mRzKlicGTPEpbiaW6u16bWlOGndKRQCPUP1596a6ibl5YCMG5hdMmBFM7JXpHA7YVZzUGxxQ/640?wx_fmt=png)
...
![](https://mmbiz.qpic.cn/mmbiz_png/icntuIQtpSJVjRR5o4mRzKlicGTPEpbiaW6BmAR3RicwkyRVnRibIp4BDMrFCLibF4VFTj5ia5vBeM7VTHq2lSx9bd1Pw/640?wx_fmt=png)
...
![](https://mmbiz.qpic.cn/mmbiz_png/icntuIQtpSJVjRR5o4mRzKlicGTPEpbiaW6G3lugBhMvC85Gg9kibdZtXRTibyFNPiangqKicDAwIa4yucgspgqftEPww/640?wx_fmt=png)

2、AliasResult关键代码如下：

![](https://mmbiz.qpic.cn/mmbiz_png/icntuIQtpSJVjRR5o4mRzKlicGTPEpbiaW6RsoFnbbiaPElIZUkt5j5tANuk2OAKmicFCpBj4ExwWLpMBdVWAXbaicqw/640?wx_fmt=png)

其中NoAlias表示两个内存对象没有任何重叠区域；MayAlias表示两个指针可能指向同一对象；PartialAlias表示两个内存对象对应的地址空间有重叠；MustAlias表示两个内存对象总是从同一位置开始。

3、ModRefResult关键代码

![](https://mmbiz.qpic.cn/mmbiz_png/icntuIQtpSJVjRR5o4mRzKlicGTPEpbiaW6X20FZl12aA74Tjaicyo4EMNPlX0MNyhEyFoQRsrBUdPELEOcC8xkFag/640?wx_fmt=png)
...
![](https://mmbiz.qpic.cn/mmbiz_png/icntuIQtpSJVjRR5o4mRzKlicGTPEpbiaW6oBt41u1GZ0dE2U84bpicHBIWoD5Qhf0BWdXibApJMVmrq2TzvmRxBbUw/640?wx_fmt=png)
...
![](https://mmbiz.qpic.cn/mmbiz_png/icntuIQtpSJVjRR5o4mRzKlicGTPEpbiaW65fqH4VE0Oz6Ubqrk9iceNibcs6IKprhCofdBXuwkGQzrSMysSYib7OmFw/640?wx_fmt=png)

##### getModRefInfo 接口

getModReInfo方法返回关于给定的指令执行是否可以读取或修改给定内存位置的信息。Mod/Ref信息具有保守性：如果一条指令可能读或写一个位置，则返回ModRef。其接口定义众多，我们以如下接口为例来进行学习。

![](https://mmbiz.qpic.cn/mmbiz_png/icntuIQtpSJVjRR5o4mRzKlicGTPEpbiaW6ZViavMIH0CJXBUb6VVITichgw6y8v4MSpwK7FkN3Ky4ibzIKqcQGsnSLQ/640?wx_fmt=png)

其接口实现如下：

![](https://mmbiz.qpic.cn/mmbiz_png/icntuIQtpSJVjRR5o4mRzKlicGTPEpbiaW698vgjZ4RDFbmT2I9KUR1FQ7DFlP2uBicof47jTSPWApdopibGLdbsdTg/640?wx_fmt=png)
...
![](https://mmbiz.qpic.cn/mmbiz_gif/icntuIQtpSJVlyv3AtZZJG8cI5EdsmQ76MGTZm1jDL28o3fhCI4DMOkyyGRsnxibTKODPFPLia7EKic2XGicf3264Ew/640?wx_fmt=gif&wxfrom=5&wx_lazy=1)

欢迎加入Compiler SIG交流群与大家共同交流学习编译技术相关内容，扫码添加小助手微信邀请你进入Compiler SIG交流群。

![](https://mmbiz.qpic.cn/mmbiz_jpg/icntuIQtpSJVlyv3AtZZJG8cI5EdsmQ76SvmL6la3icDxzRbeaghP1KmZNMOb5AVQKq2UZy8rgfKj5svhVEETdBQ/640?wx_fmt=jpeg&wxfrom=5&wx_lazy=1&wx_co=1)

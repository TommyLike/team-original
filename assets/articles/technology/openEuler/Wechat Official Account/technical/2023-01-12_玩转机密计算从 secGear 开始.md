# [玩转机密计算从 secGear 开始](https://mp.weixin.qq.com/s/IC7gprtOVRj3XrpAtRkSlQ)
...
![](https://mmbiz.qpic.cn/mmbiz_png/A0h5yD51CMZdRqiciarzNQ6qSuNp2rwkXeAb5Y6EpjAnMeBIW0GqgGjlSZJt6pSXxAB2nzmuvV13iayNCFbEtHQqQ/640?wx_fmt=png)
...
![](https://mmbiz.qpic.cn/mmbiz_png/A0h5yD51CMZdRqiciarzNQ6qSuNp2rwkXerb71DliczicuRPOQfkNRdAuUlT2DujKEHyqRsTarbicFrs3GjXg5m9dfw/640?wx_fmt=png)
...
![](https://mmbiz.qpic.cn/mmbiz_png/A0h5yD51CMZdRqiciarzNQ6qSuNp2rwkXerwQxw6FIMUqScmSTPZfSIU8swOPQ0t129yHXRfiaQWEZmsd1KgGCiafg/640?wx_fmt=png)
...
![](https://mmbiz.qpic.cn/mmbiz_png/A0h5yD51CMZdRqiciarzNQ6qSuNp2rwkXeR4VNX2EWehCPmkwgbiatrh6AruxGLyjcexkaj6ohZvzQvCp6v8Y9IaA/640?wx_fmt=png)

如图所示，全密态数据库的特点如下：

1. 数据文件以密文形式存储，不存储密钥明文信息。
2. DB 数据密钥保存在客户端。
3. 客户端发起查询请求时，在服务端 REE 侧执行密态 SQL 语法得到相关密文记录，送入 TEE 中。
4. 客户端通过 secGear 安全通道将 DB 数据密钥加密传输到服务端 TEE 中，在 TEE 中解密得到 DB 数据密钥，用 DB 数据密钥将密文记录解密得到明文记录，执行 SQL 语句，得到查询结果，再将 DB 数据密钥加密后的查询结果发送给客户端。
...
![](https://mmbiz.qpic.cn/mmbiz_png/A0h5yD51CMZdRqiciarzNQ6qSuNp2rwkXe8vwPFrpxxLdvtcic9GW6T5WibWJaibCBd8KpOE4GYAnTo7TTsmq6ebnng/640?wx_fmt=png)

如图所示，具体查询流程如下：

1. 用户在数据中心 A 下发跨域查询请求，openLooKeng 的 Coordinator 根据查询 SQL 及数据源分布，拆解下发执行计划到本地工作节点以及边缘数据中心的 coordinator，边缘数据中心的 coordinator 再下发到本地工作节点。
2. 各工作节点执行计划，得到本地计算结果。
3. 边缘数据中心通过 secGear 安全通道将本地计算结果加密后经网络传到数据中心 A 的 REE 侧，并中转到 TEE 中，在 TEE 中解密计算结果。
4. 数据中心 A 在 TEE 中对数据中心 A、B、C 的计算结果执行聚合计算，得到最终执行结果，并返回给用户。
...
![](https://mmbiz.qpic.cn/mmbiz_png/A0h5yD51CMZdRqiciarzNQ6qSuNp2rwkXeawOZiaXrzFQuT7SfKiceJwqsJ2jOb6WHuNo2HfvFoT1qyial3zvvEptsA/640?wx_fmt=png)

1. 拥有属性的参与方（Follower 方）都会持有一个底层网络，参与方属性输入底层网络得到中间结果，再将中间结果发送给拥有标签的参与方（Leader 方）。
2. Leader 方使用各参与方的中间结果和标签来训练顶层网络，再将计算得到的梯度回传给各参与方来训练底层网络。
...
![](https://mmbiz.qpic.cn/mmbiz_png/A0h5yD51CMZdRqiciarzNQ6qSuNp2rwkXee0ML3UbHlE6AErhVpMAiaxvROwXqGeQTUIJbnNNujCzRkTIBMEib6aUA/640?wx_fmt=png)

1. Follower 方的中间结果通过 secGear 的安全通道加密后传输到 Leader 方，Leader 方非安全世界接收到加密的中间结果后中转到安全世界，在安全世界通过安全通道接口解密。
...
如果您对机密计算技术感兴趣，也欢迎加入我们。添加以下小助手微信，回复 secGear 入群。

![](https://mmbiz.qpic.cn/mmbiz_jpg/A0h5yD51CMZdRqiciarzNQ6qSuNp2rwkXe5ibMoJC4C7JBu76mqySshVRXMIZljUZVP27icwjQEw8OKujib8nh7Y1DA/640?wx_fmt=jpeg)

## 引用

序号引用链接\[1]Confidential Computing Consortium, “A Technical Analysis of Confidential Computing,”: https://confidentialcomputing.io/wp-content/uploads/sites/85/2022/01/CCC-A-Technical-Analysis-of-Confidential-Computing-v1.2.pdf.\[2]木兰宽松许可证: http://license.coscl.org.cn/\[3]数字认证: https://mp.weixin.qq.com/s?\_\_biz=MzA4NDUzMzY2Mw==&chksm=87ed5619b09adf0fe8a509dcee9d7413f3dbd1ec6f39d0d247272c1caf6d754d943be18133af&idx=1&mid=2650554089&sn=5725062bb6a1712c224a2f31d4558d32\[4]Mindspore: https://www.mindspore.cn/federated/docs/zh-CN/master/secure\_vertical\_federated\_learning\_with\_TEE.html

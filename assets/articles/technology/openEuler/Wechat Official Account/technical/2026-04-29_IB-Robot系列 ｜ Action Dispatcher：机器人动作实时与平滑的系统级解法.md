# [IB-Robot系列 ｜ Action Dispatcher：机器人动作实时与平滑的系统级解法](https://mp.weixin.qq.com/s/9fRPe7fDvx93vRC1zOXKuQ)

原创*吴小强*[OpenAtom openEuler](javascript:void%280%29;)*2026-04-29 18:25:21广东*
...
# 在前一篇《[IB-Robot系列 | 具身Claw：具身智能OS的大小脑协同具身智能体](https://mp.weixin.qq.com/s?__biz=MzkyMjYzNjU0Ng%3D%3D&mid=2247519446&idx=1&sn=027585402f2d0702cccf8364f16e3397&scene=21#wechat_redirect)》关于系统级融合思路的探讨之后，本文将聚焦一个更为底层的工程痛点：具身模型输出的动作，如何在真实的机器人硬件上连续、稳定、无顿挫地执行？这背后发挥核心作用的，**正是系统中的关键组件：Action Dispatcher。**
...
![](https://mmbiz.qpic.cn/mmbiz_png/rxr9tddEHOvWvo4mSx9ImuA5DMVaRicoQHFPaeqZOZfCL7awOkEiazFj5F820dvsl1UaHWNiaXMGQRwzfKK6aavayuicxSmb3q1SuF4t6UdLh8Y/640?wx_fmt=png&from=appmsg)

▲Action Dispatcher 在整体链路中的位置示意图

**01**

**核心挑战：从离散推理到连续执行的实时平滑过渡**

若仅将模型输出的动作直接下发给机械臂，系统固然能够运转，但在真实的物理环境中会立刻面临以下矛盾：

- **推理存在非零延迟**：模型在计算下一段动作时，物理世界的机器人并不会停下来等待。
- **底层控制是高频连续的**：我们的 Action Dispatcher 必须维持固定频率的控制循环（默认 **100Hz**）。
- **模型输出具有分块特性**：单次推理的结果通常不是”下一个时间点的状态”，而是一整段轨迹序列（Chunk）。
...
![](https://mmbiz.qpic.cn/mmbiz_png/rxr9tddEHOuNBEfyNe3icM5UKVO80Rvj7BxOHBJNiaVGxy4TIRNnUposNz9ruAAI31FHKeJiauIaYxOUtFryWiaeMbEw2MOoeGMcZWKZfKtB78I/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/rxr9tddEHOtywWZwGFMNRqS7MUxLITWhe8Ilx4oWsoojOoEIZj1A5JnYoQLtnkbnc3uPXSEMR2EyuHcQlO3agx8ucbeMDXvJXmJX8fshDHU/640?wx_fmt=png&from=appmsg)

▲跨帧平滑示意图

在系统中，计算图示中a1~a3偏移量的核心指标称为 actions\_executed\_during\_inference。它绝非经验估算，而是通过严格的”推理发起时的队列长度”减去”结果返回时的队列长度”计算得出。换言之，**系统能够精准获悉在等待新推理结果的这段时间窗口内，物理执行流实际向前推进了多少步**，从而实现新旧 chunk 之间的完美时间轴对齐。

**未启用跨帧平滑**

![](https://mmbiz.qpic.cn/mmbiz_gif/rxr9tddEHOsOEZWes5AmLWaLTrlqqKsZhzR5GS12I4gSkBkgiczKnNTSiaNee5ibkzCqXD2xF2ICPq1hR3ibib9LWfJPxuJHJUJKPesmNeagr6bQ/640?wx_fmt=gif&from=appmsg)

**启用跨帧平滑**

![](https://mmbiz.qpic.cn/sz_mmbiz_gif/rxr9tddEHOvAr9J7vlHia5jECxyphwGiaKd4mTr8eEDiakEYLPqQld8zqzBF2QFibnVTlMJqaqfG2iaJBiaI6QPt3z7G7jk237csbNgiaiaL5sp1wjs/640?wx_fmt=gif&from=appmsg)

**跨帧平滑效果对比：上面**未启用跨帧平滑，机器人动作出现明显反应迟缓，控制频率受限于模型推理延迟；下面启用 Action Dispatcher 跨帧平滑后，动作连贯流畅、控制频率较高，同时闭环控制及时。

**04**

**路线对比：系统级融合 vs.算法级RTC**
...
![](https://mmbiz.qpic.cn/sz_mmbiz_gif/rxr9tddEHOsSibvh1fSEZJibNoiasNbufAgq6iaeDyl7ASBOXickad5OFV55YDJc47fTB7YVKWOSDBEibLZkAtk9kneqE0YdZbsZrticcXlxxsOEho/640?wx_fmt=gif&from=appmsg)

![](https://mmbiz.qpic.cn/sz_mmbiz_gif/rxr9tddEHOvxx9hN68IvNdSe9ES8ArowFRprsXVbNxvdyKuDEymh4Hj9uomXje7CjUPibF6EK38EmM36OFtCwKA5X89N6gyyTXXAicQZtGLv4/640?wx_fmt=gif&from=appmsg)
...
在运行时冻结已执行前缀，通过 In-painting 或软掩码技术强行引导模型续写后续动作。（如 Pi 的方案）

无需重训模型，对运行时的实际 delay 感知敏锐。

**推高算力负担**：推理过程伴随额外的梯度引导或掩码计算开销。

**训练期** **RTC**

在训练阶段显式模拟各种推理延迟分布，迫使模型原生具备”基于前缀顺滑续写”的能力。

连续性内化入模型权重，释放了推理算力。

**环境泛化受限**：强依赖预设的 delay 先验，面对未覆盖的物理波动时灵活性变弱。
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/rxr9tddEHOuJxVcwEdnnd4g3XzIZu7Q5ecibQy6XVXRDOMgOLx1VZIGbBq9vJZUNHNhMcGJrFsmz8sk9htpEugNjefAdZlF9WyYDJHiav7beQ/640?wx_fmt=png&from=appmsg)

▲Action Dispatcher 内部结构示意图

**05**

**云边端协同演进：从静态平滑到动态系统级调度**
...
因此，当前在 Action Dispatcher 中构建的跨帧平滑机制，绝不仅是一个局部的工程 Trick，而是为未来高并发云边端协同打下底座。只有先确保”指令无缝接力”，未来系统才能拥有”按需动态调节控制节奏”的自由度。

**06**

**IB Robot的架构选择：具身模型与系统调度的深度解耦**

探讨至此，回到最核心的问题：面对业界花样百出的算法方案，具身系统究竟笃定了怎样的架构演进路线？答案非常明确：**坚定选择一条”模型深度解耦的系统级调度架构”。**
...
1. **极致的即插即用（Plug-and-Play）**：我们无需干涉上游接入的是基于 Diffusion 的 ![](https://mmbiz.qpic.cn/mmbiz_gif/rxr9tddEHOsDDEVx9ZG0pWfGWotica6pxPJXiaKABl1uJcpd8FcHf4fP75kcxXCj40SgugABX0BzjZLVOEKSD8iavicerkogdfurwJwNCrM1EKM/640?wx_fmt=gif&from=appmsg) 系列、自回归 Transformer，亦或是任何开源 VLA 大模型。开发者无需为了适配硬件环境而去魔改损失函数，或在推理期引入高昂的附加算力。只要模型能输出动作序列，我们的 Dispatcher 就能精准接管、平滑缝合。
...
3. **拥抱规模化落地的终极解法**：在实验室跑通单一 Demo 并非难事，但当具身智能走向真实的商业化部署，必然要面对”云端大模型 + 边缘小模型 + 端侧高频控制”的复杂异步拓扑。我们的架构提前锁定了这一未来场景，原生提供了一整套”系统执行策略旋钮”（包括 temporal\_ensemble\_coeff、动态水位线、Overlap 自适应调节等）。

# [「转」破解分布式计算多租困局：openYuanrong多租隔离](https://mp.weixin.qq.com/s/4jbL9Aivs1k0xmOZLlODWQ)

*望山*[OpenAtom openEuler](javascript:void%280%29;)*2026-03-04 17:56:00中国香港*
...
![](https://mmbiz.qpic.cn/mmbiz_png/qCr335HdbQbRBZ8nHZR6OuYr9FcvZTvRy5gdxbBA0cibFhOgtVmSJM2p90WibBzZASmlDibuDPNV2RlxO4EbxmFqTxibCgBU3u55Su7qcdObKN0/640?wx_fmt=png&from=appmsg)
...
- 租户运行隔离：支持按需配置不同的隔离模式，追求极致的资源利用率且隔离要求不高时，可采用“进程隔离”模式；需确保业务绝对稳定、避免干扰时，可选择“单 Pod 单函数”模式；希望保证租户间隔离的同时控制资源开销，可选择“单 Pod 多函数”。
  ...
  ![](https://mmbiz.qpic.cn/mmbiz_png/qCr335HdbQb2ibc1FtdxuT8kVUwhcrkgjgNecw38xTIg8oqSicoqoPo20R6awHrkDqcJRTTORjAErFtqvib8IEJ6uKQoiaMf1wqLIYnIST9OabI/640?wx_fmt=png&from=appmsg)
...
- 租户调用隔离：每个租户分配唯一的身份凭证 Token 及 CA 根证书，经由基于证书认证的 TLS 安全通道下发至租户函数运行时。在通信过程中，FunctionProxy 验证函数实例的 Token，函数实例验证 FunctionProxy 的证书，从而实现双向认证与 TLS 加密传输。该机制支持按需配置启用或关闭，采用可插拔架构，便于与外部认证鉴权系统对接。
  ...
  ![](https://mmbiz.qpic.cn/mmbiz_png/qCr335HdbQa1XVPLc2M1jWJMicKDPJhmmJY90Qoo0cMkkuWX9d5AdCnDOe3aIL3sFVrtER0icCrpaxtbFKTIotUG8QfWxRgjiaZDnnvzLsQVtY/640?wx_fmt=png&from=appmsg)
...
- 租户数据隔离：数据系统对租户数据进行了物理隔离，函数在访问数据系统共享内存时，可基于租户 Token 与 DataWorker 进行认证鉴权。该机制支持按需配置，启用后能够确保各租户仅可访问和操作本租户数据，从而实现租户间的数据强隔离，灵活适应不同业务场景对安全与性能的要求。
  ...
  ![](https://mmbiz.qpic.cn/mmbiz_png/qCr335HdbQasB0wZz8icWiaaR26Cw7fe6tM1ovb3FPhvceG5tqcicP0mWqDia5kgrC24EMCx5ORYnjQX8ROiasQsjOYC4ibKW9p9QF01NWgRC1IRg/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/qCr335HdbQbNAhghfQamMMAMUiacVdK62eblicKQRelbicafm40TaMfcNyIeicG3Zzyh4vuChrSyNCN2KblwzdjMgqcA0pmicBialL5YSBjsGwf10/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/qCr335HdbQY5SbQnicwdls6hWrySIDbllIJJHkWycyZbaV64RZcicAoKxFwUx8SeMpuRQpVYicibo4iaqsAh9X5elcQUF7icG220kb6JicuphfRwaw/640?wx_fmt=png&from=appmsg)

# [openEuler 24.03 LTS 特性解读 ｜ 动态复合页](https://mp.weixin.qq.com/s/2rVUFXxL83Q1n_aM1gKDzQ)

[OpenAtom openEuler](javascript:void%280%29;)*2024-07-09 17:26:18中国香港*

在OpenAtom openEuler（简称"openEuler"） 20.03 LTS 历史版本中，ARM64架构下默认基础页大小配置为64KB，相比传统的4KB基础页，在访存方面，64K页面能带来更快的page tale walk、更低的TLB miss，在存储IO路径上也能提升读写效率，因此64K基础页能够提升数据库等部分场景下的性能。然而，64K基础页却面临南北向生态问题（部分应用软件和外设驱动需要重新适配或重新编译）和内存底噪开销问题（64K相比4K粒度更粗，内存利用率更低，部分场景会带来巨大的内存消耗）。为此，openEuler在22.03版本将ARM64的默认基础页大小配置改回了4K，在性能上64K基础页有较大优势的场景，则可以单独编译64K基础页的二进制版本来支持，或者采用大页技术，虽然传统大页技术同样能够提升访存性能，但是它面临着使用不够灵活（hugetlb）和性能QoS无法保证（THP）的问题。

![](https://mmbiz.qpic.cn/mmbiz_png/jqTxnpIZ2mATzNFrXoraEiaSJExDicbme1JIvTCtOgEudqFKOtkheic2ib8f23ByMWHabsQruWl65NZGyvEsiaBXWeg/640?wx_fmt=png&from=appmsg)

**ARM64内存地址翻译对比**

那么有没有一种技术，可以在一个内核二进制版本中，实现4K基础页的南北向生态兼容，以及64K页的性能提升，同时应用能够无感使用，并能够有效控制内存底噪？openEuler 24.03版本带来的**动态复合页（large folio）技术**正在解决这个难题。
...
openEuler 24.03版本基于linux 6.6 LTS版本作为基线进行演进，linux 6.6基线版本已经完成了部分模块对基础folio（4K）的重构，包括：内存分配、内存回收、pagecache、部分文件系统（xfs等），而**对于large folio的支持则普遍缺失**。为此，华为内核团队除了在linux社区中积极贡献folio相关补丁的同时，也在openEuler 24.03配套的6.6内核中实现了内存管理子系统和文件系统对于large folio的大量支持和软硬协同优化，并提供灵活的控制策略，完善了动态复合页技术架构：

![](https://mmbiz.qpic.cn/mmbiz_png/jqTxnpIZ2mATzNFrXoraEiaSJExDicbme1TMsTHMcOYnsIeUQfPhicZwk5X3JXYOlIIhT5oFjk9zy8P6a0A2wGiaOg/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/jqTxnpIZ2mATzNFrXoraEiaSJExDicbme1LibRNEQibEnCXIMCo12UlIUHGuQuK7UhORLUk1FC7Yx8psIClR9zg7eg/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/jqTxnpIZ2mATzNFrXoraEiaSJExDicbme1icOcfNFfcibUQGFiaHKplEUt6akeqERDpRC0Glm3h6KGdsnoG0d6NqMIQ/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/jqTxnpIZ2mATzNFrXoraEiaSJExDicbme11SvNSkjlZHFNN2Rh2O4HcDALp3kbfUDBI3Sic3KAzcBnficjHsc4vdcA/640?wx_fmt=png&from=appmsg)

## Kafka收益测试

打开动态复合页后，测试kafka benchmark结果：producer带宽提升26%，consumer带宽提升11%

![](https://mmbiz.qpic.cn/mmbiz_png/jqTxnpIZ2mATzNFrXoraEiaSJExDicbme1zicP2RVwEoR2GsRpBaneBT4NHzqqjeNhZ1WtbjZB97iaQmljMicwNfMaA/640?wx_fmt=png&from=appmsg)

## MySQL收益测试

打开动态复合页后，测试MySQL benchmark（sysbench），受益于代码段可以用户态无感得合成2M大页从而使**iTLB miss率最高降低10倍，sysbench整体结果性能提升3%+**

![](https://mmbiz.qpic.cn/mmbiz_png/jqTxnpIZ2mATzNFrXoraEiaSJExDicbme18P9nOHOkzKN2KyVupxgmrYPoyvWHKlibmviadDBgNYS53RtJElBSEBQA/640?wx_fmt=png&from=appmsg)

## IO基础性能benchmark收益测试

打开动态复合页后，测试FIO读性能平均提升59%，写性能平均提升239%

![](https://mmbiz.qpic.cn/mmbiz_png/jqTxnpIZ2mATzNFrXoraEiaSJExDicbme1Tm14EhOh0oCQhZLCmCPp8dVFmXl5a8ibiaXXSpx09AmRMddBzdSI3Oag/640?wx_fmt=png&from=appmsg)

![](https://mmbiz.qpic.cn/mmbiz_png/jqTxnpIZ2mATzNFrXoraEiaSJExDicbme1iaZs3rBn7icvwrNfJgGCSGQDogMPhVCm4QH8dYBCQddRNnwF2oEwcS1g/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/jqTxnpIZ2mATzNFrXoraEiaSJExDicbme11O9svztZODCKUibMwUeytKBODvM26ukw2wCdiaw2GNjSZhiaL7K8xm1Dw/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/jqTxnpIZ2mATzNFrXoraEiaSJExDicbme1D49fupyHmzjkb6VnMOEe9tcNgXYAE8dTaOafqgJLibibg9EtByhMd0ibQ/640?wx_fmt=png&from=appmsg)

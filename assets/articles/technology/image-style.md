...
> 一致、可扫描的视觉风格。
>
> **已用 Gemini `gemini-3-pro-image` 实测验证**（见 `style-tests/`）：
> 同一套 STYLE 提示词跨主题输出高度一致，文字渲染清晰。
...
- `sources/<topic>/images/` — 从原文抽取的样图（ground truth）
- `style-tests/` — 用本规范 STYLE 块经 Gemini 生成的验证图（gen-attention /
  gen-moe-routing / gen-quantization），可与 `sources/` 对照确认一致性
